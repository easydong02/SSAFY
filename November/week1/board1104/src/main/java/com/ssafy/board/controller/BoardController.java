...
public class BoardController {
	...
	@ApiOperation(value = "게시판", notes = "게시판의 <big>한개의</big>를 수정한다.")
	@ApiResponses({ @ApiResponse(code = 200, message = "게시판글 OK!!"), @ApiResponse(code = 404, message = "페이지없어!!"),
		@ApiResponse(code = 500, message = "서버에러!!") })
	@PutMapping(value = "/")
	public ResponseEntity<?> update(@RequestBody BoardDto board) {
		...
		try {
			...
			BoardDto boardDto = boardService.getBoard(board.getArticleNo());
			if(boardDto != null) {
	...
	@ApiOperation(value = "게시판", notes = "게시판의 <big>한개의</big>을 삭제해 줍니다.")
	@ApiResponses({ @ApiResponse(code = 200, message = "게시판글 OK!!"), @ApiResponse(code = 404, message = "페이지없어!!"),
			@ApiResponse(code = 500, message = "서버에러!!") })
	@DeleteMapping(value = "/{articleNo}")
	public ResponseEntity<?> boardelete(@PathVariable int articleNo) {
		logger.debug("ABoardController boardetail");
		try {
