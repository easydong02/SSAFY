...
public class MemberDaoImpl implements IMemberDao {
	...
	public int regist(Member member) throws Exception {
		...
		sql+= " values(?,hex(aes_encrypt(?,sha2('abc',256))),?,?,?,?,?)";
	...
	public Member login(String userId, String userPwd) throws Exception {
		...
		sql+="where user_id=? and convert(aes_decrypt(unhex(password),sha2('abc',256)) using utf8) =?";
	...
	public Member findPwd(String userName, String userId) throws Exception {
		...
		String sql = "select convert(aes_decrypt(unhex(password),sha2('abc',256)) using utf8) from members \n";
		...
		try {
			...
			if(rs.next()) {
				member=new Member();
				member.setUserPwd(rs.getString(1));
	...
	public int update(Member member) throws Exception {
		String sql = " update members set user_id=?,password=hex(aes_encrypt(?,sha2('abc',256))),name=?,email=?,age=?,role=?,delflag=0 \n";
		sql+= " where user_id=? ";
		Connection conn = null;
		PreparedStatement psmt = null;
		int count = 0;
