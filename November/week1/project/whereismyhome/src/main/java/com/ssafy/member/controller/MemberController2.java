...
public class MemberController2 extends HttpServlet {
	...
	@Autowired
	public MemberController2(MemberService memerService, InterestService interestService,IDongService dongService ) {
...
//	    				dongs.add(d);
	...
	private String getList(HttpServletRequest request, HttpServletResponse response) {
		try {
			List<MemberDto> members = memberService.getList();
			if(members!=null) {
				HttpSession session = request.getSession();
...
//		member.setEmail(emailId+"@"+emailDomain); -----------------------주석
		try {
...
//				String[] email = member.getEmail().split("@");  -----------------------주석
//				request.setAttribute("emailId", email[0]); -----------------------주석
//				request.setAttribute("emailDomain", email[1]); -----------------------주석
	...
	private String doFindPwd(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
//			MemberDto member = memberService.findPwd(userName,userId); -----------------------주석
...
//			System.out.println(member.toString());
	...
	private String doLogin(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
//			MemberDto member = memberService.login(userId, userPwd); -----------------------주석
			MemberDto member =null;
			if(member!=null) {
				HttpSession session = request.getSession();
